...
class DummyCLAP:
    ...
    def get_audio_embedding_from_data(
        self, data=None, *, x=None, use_tensor=False
    ):
        if x is not None:
            data = x
...
class SearchEngine:
    def __init__(self, embeddings_path):
        ...
        try:
            ...
            print(f"Warnung: CLAP-Modell konnte nicht geladen werden: {e}")
        ...
        self.embedding_tensors = torch.tensor(
    ...
    def search(self, prompt: str, top_k: int = 5):
        ...
        # Berechne Kosinus-Ähnlichkeit zwischen Text-Embedding
        # und allen Audio-Embeddings
        ...
        top_k_values, top_k_indices = torch.topk(
            similarities, k=min(top_k, len(self.file_paths)))
...
def main():
    ...
    try:
        ...
        print("="*60)
        print(
            "Geben Sie einen Text-Prompt ein, um passende Audio-Dateien zu "
            "finden."
        )
        print(
            "Beispiele: 'dark industrial kick', 'melodic synth pad', "
            "'punchy snare'"
        )
        ...
        while True:
            try:
                # Benutzereingabe
                prompt = input(
                    "Prompt eingeben (oder 'exit' zum Beenden): ").strip()
        ...
        print(
            f"Fehler: Embeddings-Datei nicht gefunden: {embeddings_file}"
        )
        print(
            "Bitte führen Sie zuerst "
            "'python ai_agents/minimal_preprocessor.py' aus."
        )
    except Exception as e:
