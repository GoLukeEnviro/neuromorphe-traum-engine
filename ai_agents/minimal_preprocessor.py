...
Erstellt CLAP-Embeddings für Audio-Dateien und speichert sie in einer
kompakten Binärdatei.
"""
...
class DummyCLAP:
    ...
    def get_audio_embedding_from_filelist(self, x=None, use_tensor=False):
        """Returniert Null-Embeddings für eine Liste von Dateien."""
        if x is None:
            return []
        result = [np.zeros(512) for _ in x]
        if use_tensor:
            return [torch.tensor(r) for r in result]
        return result
...
class MinimalPreprocessor:
    ...
    def _process_batch(self, file_batch):
        ...
        try:
            ...
            for file_path in file_batch:
                try:
                    embedding = (
                        self.clap_model.get_audio_embedding_from_filelist(
                            x=[file_path],
                            use_tensor=False,
                        )[0]
                    )
    ...
    def run(self):
        ...
        Findet Audio-Dateien, verarbeitet sie in Batches und speichert die
        Embeddings.
        """
        ...
        print(
            f"Speichere {len(all_results)} Embeddings in: {self.output_path}")
